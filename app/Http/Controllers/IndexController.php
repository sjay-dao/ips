...
class IndexController extends Controller
{
    ...
    public function searchInIpdata(Request $request){
        ...
        $sql_str = "SELECT * from `ips system`.`ips` where $ip_string   $status_string (ips.name Like '$request->search_word' OR doc_code Like '$request->search_word'OR project_title Like '$request->search_word'OR date_filed Like '$request->search_word'OR funding_source Like '$request->search_word'OR reg_no Like '$request->search_word' OR author_r_inventor Like '$request->search_word' OR ips.status Like '$request->search_word'OR date_approved Like '$request->search_word' OR duration Like '$request->search_word'OR project_cost Like '$request->search_word'OR funding_source Like '$request->search_word'OR income_gross Like '$request->search_word') AND (date_filed > '$request->yearfrom-01-01' AND date_filed < '$request->yearto-12-31') 
    ...
    public function countFiledAndApproved(Request $request){
        ...
        $sql_str_min = "SELECT * from ips order by date_filed asc limit 0,1";
        $sql_str_max = "SELECT * from ips order by date_filed desc limit 0,1";
        ...
        for($x=$min_year; $x<=$max_year; $x++) {
            ...
            $sql_str = "SELECT count(id) as filed from `ips system`.ips where type_id= 1 and date_filed >=  STR_TO_DATE('".$datefrom.
            "', '%Y-%m-%d') and date_filed <=  STR_TO_DATE('".$dateto."', '%Y-%m-%d')";
            $patent_filed = DB::select($sql_str);
            $sql_str_ap = "SELECT COUNT(id) AS approved FROM `ips system`.ips AS a WHERE type_id = 1 AND (a.status LIKE '%Approved%' OR a.status LIKE '%Granted%') and date_filed >=  STR_TO_DATE('".$datefrom.
            "', '%Y-%m-%d') and date_filed <=  STR_TO_DATE('".$dateto."', '%Y-%m-%d')";
            $patent_approved = DB::select($sql_str_ap);

            //get the count of filed and approved for Utility Model
            $sql_str = "SELECT count(id) as filed from `ips system`.ips where type_id= 2 and date_filed >=  STR_TO_DATE('".$datefrom.
            "', '%Y-%m-%d') and date_filed <=  STR_TO_DATE('".$dateto."', '%Y-%m-%d')";
            $um_filed = DB::select($sql_str);
            $sql_str_ap = "SELECT COUNT(id) AS approved FROM `ips system`.ips AS a WHERE type_id = 2 AND (a.status LIKE '%Approved%' OR a.status LIKE '%Granted%') and date_filed >=  STR_TO_DATE('".$datefrom.
            "', '%Y-%m-%d') and date_filed <=  STR_TO_DATE('".$dateto."', '%Y-%m-%d')";
            $um_approved = DB::select($sql_str_ap);

            //get the count of filed and approved for Trademark
            $sql_str = "SELECT count(id) as filed from `ips system`.ips where type_id= 3 and date_filed >=  STR_TO_DATE('".$datefrom.
            "', '%Y-%m-%d') and date_filed <=  STR_TO_DATE('".$dateto."', '%Y-%m-%d')";
            $trademark_filed = DB::select($sql_str);
            $sql_str_ap = "SELECT COUNT(id) AS approved FROM `ips system`.ips AS a WHERE type_id = 3 AND (a.status LIKE '%Approved%' OR a.status LIKE '%Granted%') and date_filed >=  STR_TO_DATE('".$datefrom.
            "', '%Y-%m-%d') and date_filed <=  STR_TO_DATE('".$dateto."', '%Y-%m-%d')";
            $trademark_approved = DB::select($sql_str_ap);

            //get the count of filed and approved for Copyright
            $sql_str = "SELECT count(id) as filed from `ips system`.ips where type_id= 4 and date_filed >=  STR_TO_DATE('".$datefrom.
            "', '%Y-%m-%d') and date_filed <=  STR_TO_DATE('".$dateto."', '%Y-%m-%d')";
            $copyright_filed = DB::select($sql_str);
            $sql_str_ap = "SELECT COUNT(id) AS approved FROM `ips system`.ips AS a WHERE type_id = 4 AND (a.status LIKE '%Approved%' OR a.status LIKE '%Granted%') and date_filed >=  STR_TO_DATE('".$datefrom.
            "', '%Y-%m-%d') and date_filed <=  STR_TO_DATE('".$dateto."', '%Y-%m-%d')";
            $copyright_approved = DB::select($sql_str_ap);

            //get the count of filed and approved for All
            $sql_str_all = "SELECT count(id) as filed from `ips system`.ips where  date_filed >=  STR_TO_DATE('".$datefrom.
            "', '%Y-%m-%d') and date_filed <=  STR_TO_DATE('".$dateto."', '%Y-%m-%d')";
            $all_filed = DB::select($sql_str_all);
            $sql_str_ap_all = "SELECT COUNT(id) AS approved FROM `ips system`.ips AS a WHERE  (a.status LIKE '%Approved%' OR a.status LIKE '%Granted%') and date_filed >=  STR_TO_DATE('".$datefrom.
            "', '%Y-%m-%d') and date_filed <=  STR_TO_DATE('".$dateto."', '%Y-%m-%d')";
    ...
    public function getIpTypesCounts(){
        $data = array();
        $sql_str = "SELECT count(id) as val from `ips system`.ips";
        $sql_str2 = "SELECT COUNT(id) AS val FROM `ips system`.ips WHERE type_id = 1";
        $sql_str3 = "SELECT COUNT(id) AS val FROM `ips system`.ips WHERE type_id = 2";
        $sql_str4 = "SELECT COUNT(id) AS val FROM `ips system`.ips WHERE type_id = 3";
        $sql_str5 = "SELECT COUNT(id) AS val FROM `ips system`.ips WHERE type_id = 4";
    ...
    public function setCookieForLogin(Request $request){
        $sql_str = "SELECT * FROM users where (username = '$request->username' || email =  '$request->username') and `password` =  '$request->password' " ;
